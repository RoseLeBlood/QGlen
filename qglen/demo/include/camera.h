...
public:
    ...
    virtual void Update(qglen::GamePadState *pStates, int numDevices, double elapsedTime)
    {
         ...
         Roll( (float)(pStates->buttonR2 / elapsedTime), false);
         Roll( (float)(-pStates->buttonL2 / elapsedTime), true);
    ...
    void Roll(float roll, bool left)
    {
        m_fRoll += m_fSensetive *  (left ? roll : roll);
    }
